...
const CommentForm = ({ postId, comment, closed }: { postId: number; comment: commentType; closed: boolean }) => {
	...
	return (
		<S.CommentLi>
			<S.CommentDiv>
				<S.CommentProfileImg
					src={comment.profileImageUrl === "nonImage" ? profileImg : comment.profileImageUrl}
					alt='프로필 이미지'
				/>
				<div>
					<div style={{ display: "flex", alignItems: "center" }}>
						<S.Span fontSize={18} fontWeight='700'>
							{comment.nickname}
						</S.Span>
								...
								<S.ModalWrapper>
									...
									{isOpen && (
										<S.EditModal>
											...
											<S.ModalButton onClick={() => commentDeleteBtn(comment.id.toString())}>
												삭제하기
											</S.ModalButton>
										</S.EditModal>
									)}
