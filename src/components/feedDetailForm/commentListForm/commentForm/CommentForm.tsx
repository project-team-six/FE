	...
	return (
		<S.CommentLi>
			<S.CommentDiv>
				...
				/>
				<div>
					<S.Span fontSize={15} fontWeight='700'>
						{comment.nickname}
					</S.Span>
					<S.Span fontSize={12} fontWeight='400'>
					...
					{closed ? (
						...
						<div>
							{editCommentId === comment.id ? (
								<div>
									<S.IconButton onClick={() => commentEditBtn(comment.id.toString())}>
										<BsCheckSquare size={20} style={{}} />
									</S.IconButton>
									<S.IconButton onClick={editCancelBtn}>
										<RiDeleteBack2Line size={20} style={{}} />
									</S.IconButton>
								</div>
							) : (
								<div>
									<S.IconButton onClick={editSelectBtn}>
										<S.IconImg src={edit} alt='edit' />
									</S.IconButton>
									<S.IconButton onClick={() => commentDeleteBtn(comment.id.toString())}>
