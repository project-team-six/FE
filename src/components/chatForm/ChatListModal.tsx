			...
			{modalState && myRooms && (
				<S.ModalLayout>
					<S.ChatLayout>
						<S.HeaderSection>
							<S.HeaderDiv>
								<S.Span fontSize={17} fontWeight='700'>
									채팅 목록
								</S.Span>
							</S.HeaderDiv>
							<S.HeaderButton onClick={() => modalHandle(!modalState)}>
								<S.CloseImg src={chatClose} alt='close' />
							</S.HeaderButton>
						</S.HeaderSection>
						<S.ContentSection>
							<ul>
										...
										<S.Li key={r.roomId}>
											<S.LiButton onClick={() => handleClickChat(r.roomId)}>
												...
												<S.Section>
													<S.SpanDiv>
														<S.TitleDiv>
															<S.Span fontSize={15} fontWeight='700' MarginRight={10}>
																{r.name}
															</S.Span>
														</S.TitleDiv>
														<S.Span fontSize={10} fontWeight='400' fontColor='B1B1B1'>
															{time}
														</S.Span>
													</S.SpanDiv>
													<S.ContentDiv>
														<S.Span fontSize={12} fontWeight='500' fontColor='7C7C7C'>
															{r.lastMessage}
														</S.Span>
													</S.ContentDiv>
												</S.Section>
											</S.LiButton>
											<S.DeleteButton onClick={() => handleClickDeleteBtn(r.roomId)}>
					...
					{selectChat && (
						<ChatDetailModal
							...
							isFeed={false}
							modalState={isDetail}
							modalHandle={setIsDetail}
							preModalState={modalState}
							setPreModalState={modalHandle}
						/>
					)}
				</S.ModalLayout>
			)}
